...
В контексте Vue компилятор шаблонов применяет _статическое поднятие_ для повышения производительности. При преобразовании шаблона в функцию рендеринга виртуальные узлы (VNodes), соответствующие статическому содержимому, могут быть созданы один раз и затем использованы повторно. Эти статические узлы называются поднятыми, потому что они создаются вне функции рендеринга, до её запуска. Аналогичная форма поднятия применяется к статическим объектам или массивам, которые генерируются компилятором шаблонов.
