...
_Рендер-функция_ — это часть компонента, которая генерирует VNodes (узлы Virtual DOM), используемые во время отрисовки. Шаблоны компилируются в рендер-функции.
...
Термин _Virtual DOM_ не является уникальным для Vue. Это общий подход, используемый несколькими веб-фреймворками для управления обновлениями пользовательского интерфейса.
...
Манипуляции с DOM являются основным узким местом в производительности. Virtual DOM предоставляет одну из стратегий для управления этим.

Вместо того чтобы создавать узлы DOM напрямую, компоненты Vue генерируют описание того, какие узлы DOM они хотели бы получить. Эти дескрипторы представляют собой обычные объекты JavaScript, известные как VNodes (узлы Virtual DOM). Создание узлов VNodes обходится относительно недорого.
...
Узел _VNode_ — это _узел Virtual DOM_. Их можно создать с помощью функции [`h()`](/api/render-function.html#h).

Дополнительные сведения см. в разделе [Virtual DOM](#virtual-dom).
