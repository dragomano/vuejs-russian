...
Минорные и мажорные релизы обычно проходят через несколько стадий предварительного выпуска: **alpha**, **beta** и **release candidate (RC)**. Количество и тип предварительных релизов зависят от масштаба изменений. Например, минорный релиз с небольшими обновлениями может ограничиться только стадией beta, тогда как мажорный релиз обычно включает все три стадии, чтобы обеспечить тщательное тестирование и получить отзывы сообщества.

Вы можете установить последние предварительные релизы из npm с помощью `npx install-vue@alpha`, `npx install-vue@beta` или `npx install-vue@rc`. Чтобы протестировать изменения, которые ещё не вошли в помеченные предварительные релизы, можно использовать временные preview-сборки непрерывных релизов: каждый коммит в репозиторий [vuejs/core](https://github.com/vuejs/core) публикуется в таком формате и доступен для установки через `npx install-vue@edge`.
