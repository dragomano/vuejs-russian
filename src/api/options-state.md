...
- **Подробности**
  ...
  - **`required`**: Определяет, требуется ли параметр. В непродакшен-среде будет выдано консольное предупреждение, если это значение истинно, а параметр не передан.

  - **`validator`**: Пользовательская функция-валидатор принимает значение пропса и объект всех пропсов в качестве аргументов. В режиме разработки, если эта функция возвращает ложное значение (т. е. валидация не проходит), Vue выдаст предупреждение в консоли.
