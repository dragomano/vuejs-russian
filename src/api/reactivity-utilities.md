...
- **Пример**
  ...
  // преобразуйте `props.foo` в ref, затем передайте в композабл
...
Это можно использовать в [композаблах](/guide/reusability/composables.html) для нормализации аргумента, который может быть либо значением, либо ссылкой, либо геттером.
...
- **Пример**
  ...
  Нормализация аргументов в композаблах:
...
- **Пример**
  ...
  */
  ...
  `toRefs` полезен при возврате реактивного объекта из композабла, чтобы потребляющий компонент мог деструктурировать/распространить возвращённый объект без потери реактивности:
