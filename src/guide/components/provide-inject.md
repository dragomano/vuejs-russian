...
![Диаграмма, показывающая передачу пропсов через несколько уровней компонентов только для того, чтобы они дошли до глубоко вложенного дочернего компонента](./images/prop-drilling.png)
...
![Диаграмма, показывающая механизм provide/inject, при котором родительский компонент предоставляет зависимость, а глубоко вложенный дочерний компонент может напрямую внедрить её, минуя промежуточные компоненты](./images/provide-inject.png)
