...
![схема слотов с ограниченной областью действия](./images/scoped-slots.svg)
...
Вы можете представить себе слот как функцию, передаваемую в дочерний компонент. Затем дочерний компонент вызывает его, передавая параметры в качестве аргументов:
...
На самом деле, это очень близко к тому, как компилируются слоты с ограниченной областью действия, и как вы будете использовать такие слоты в своих [функциях рендеринга](/guide/extras/render-function).
...
### Именованные слоты с ограниченной областью действия {#named-scoped-slots}
...
Если вы смешиваете именованные слоты со слотом с ограниченной областью действия по умолчанию, вам нужно использовать явный тег `<template>` для слота по умолчанию. Попытка разместить директиву `v-slot` непосредственно на компоненте приведёт к ошибке компиляции. Это необходимо для того, чтобы избежать двусмысленности в отношении области действия параметра слота по умолчанию. Например:
...
Использование явного тега `<template>` для слота по умолчанию помогает понять, что реквизит `message` недоступен в другом слоте:
...
Вариант использования `<FancyList>`, который мы обсуждали выше, инкапсулирует как повторно используемую логику (извлечение данных, разбиение на страницы и т. д.), так и визуальный вывод, делегируя при этом часть визуального вывода потребительскому компоненту через слоты с ограниченной областью действия.
...
Несмотря на то, что это интересный паттерн, большинство из того, что можно достичь с помощью компонентов без отрисовки, может быть достигнуто более эффективным способом с помощью Composition API, без лишних затрат на вложенность компонентов. Позже мы увидим, как можно реализовать ту же функциональность отслеживания мыши в виде [Composable](/guide/reusability/composables).

Тем не менее, слоты с ограниченной областью действия всё ещё полезны в случаях, когда нам нужно одновременно инкапсулировать логику **и** компоновать визуальный вывод, как в примере `<FancyList>`.
