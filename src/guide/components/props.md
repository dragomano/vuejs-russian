...
## Объявление пропсов {#props-declaration}

Компоненты Vue требуют явного объявления пропсов, чтобы Vue знал, какие внешние параметры, переданные компоненту, должны рассматриваться как [передаваемые атрибуты](/guide/components/attrs).
...
В компонентах, не относящихся к `<script setup>`, пропсы объявляются с помощью опции [`props`](/api/options-state#props):
...
Пропсы объявляются с помощью свойства [`props`](/api/options-state#props):
...
Смотрите также: [Типизация пропсов компонента](/guide/typescript/options-api#typing-component-props) <sup class="vt-badge ts" />
...
Подробнее: [Типизация пропсов компонента](/guide/typescript/composition-api#typing-component-props) <sup class="vt-badge ts" />

</div>

<div class="composition-api">

## Деструктуризация реактивных пропсов <sup class="vt-badge" data-text="3.5+" /> \*\* {#reactive-props-destructure}
...
### Передача деструктурированных пропсов в функции {#passing-destructured-props-into-functions}
...
## Подробности передачи пропсов {#prop-passing-details}

### Регистр имён пропсов {#prop-name-casing}
...
### Сравнение статических и динамических пропсов {#static-vs-dynamic-props}
...
Когда `v-bind` используется вместе с явными привязками для одного и того же компонента, Vue внутри вызывает `mergeProps()` для их объединения. Стратегия объединения зависит от типа ключа:

- **Обычные пропсы** — приоритет имеет последнее значение:
...
- **Обработчики событий** — при передаче обработчиков в объекте `v-bind` [используйте соглашение об именовании ключей `onEventName`](/guide/extras/render-function#v-on). Все обработчики одного и того же события будут вызваны (см. [наследование обработчиков `v-on`](/guide/components/attrs#v-on-listener-inheritance)):
...
- **`class` и `style`** объединяются по аналогичной стратегии (см. [объединение `class` и `style`](/guide/components/attrs#class-and-style-merging)).

:::tip Совет
Полные правила объединения описаны в справочнике по API [`mergeProps()`](/api/render-function#mergeprops).
:::
...
## Валидация пропсов {#prop-validation}
