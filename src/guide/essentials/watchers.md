...
## Пример использования {#basic-example}
...
Первым аргументом `watch` могут быть различные типы реактивных «источников»: Это может быть ссылка (включая вычисляемые ссылки), реактивный объект, [геттер-функция](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Functions/get#описание) или массив из нескольких источников:
...
## Однократные наблюдатели {#once-watchers}
...
Можно использовать API [`onWatcherCleanup()`](/api/reactivity-core#onwatchercleanup) <sup class="vt-badge" data-text="3,5+" /> для регистрации функции очистки, которая будет вызвана, когда наблюдатель станет недействительным и будет запущен снова:
...
Кроме того, функция `onCleanup` также передается в обратные вызовы наблюдателя в качестве 3-го аргумента<span class="composition-api">, а в функцию эффекта `watchEffect` — в качестве первого аргумента</span>:
...
Это работает в версиях до 3.5. Кроме того, `onCleanup`, переданный через аргумент функции, привязан к экземпляру наблюдателя, поэтому он не подвержен синхронному ограничению `onWatcherCleanup`.
...
### Наблюдатели синхронизации {#sync-watchers}
