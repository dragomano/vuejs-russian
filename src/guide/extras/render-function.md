...
# Функции рендеринга и JSX {#render-functions-jsx}
...
### Создание узлов Virtual DOM {#creating-vnodes}

Vue предоставляет функцию `h()` для создания узлов Virtual DOM:
...
Полученный узел Virtual DOM имеет следующую форму:
...
Помимо возврата одного узла Virtual DOM, вы также можете возвращать строки или массивы:
...
### Узлы Virtual DOM должны быть уникальными {#vnodes-must-be-unique}
...
### Использование узлов Virtual DOM в `<template>` {#using-vnodes-in-template}
...
<template>
  <!-- Через <component /> -->
  <component :is="vnode">Привет</component>

  <!-- Или напрямую в виде элемента -->
  <vnode />
  <vnode>Привет</vnode>
</template>
```

Объект `vnode` был объявлен в `setup()`, и его можно использовать как обычный компонент при рендеринге.

:::warning Предупреждение
`vnode` представляет уже созданный результат рендеринга, а не определение компонента. Использование `vnode` внутри `<template>` **не создаёт новый экземпляр компонента**, а рендерит `vnode` как есть.

Этот паттерн следует использовать с осторожностью и он **не является заменой** обычным компонентам.
:::
...
Чтобы создать узел Virtual DOM для компонента, первым аргументом, передаваемым в `h()`, должно быть определение компонента. Это означает, что при использовании рендер-функций нет необходимости регистрировать компоненты — вы можете просто использовать импортированные компоненты напрямую:
...
В рендер-функциях доступ к слотам можно получить из контекста `setup()`. Каждый слот в объекте `slots` представляет собой **функцию, возвращающую массив узлов Virtual DOM**:
...
Пользовательские директивы могут быть применены к узлу Virtual DOM с помощью [`withDirectives`](/api/render-function#withdirectives):
...
С использованием Composition API, при применении [`useTemplateRef()`](/api/composition-api-helpers#usetemplateref) <sup class="vt-badge" data-text="3.5+" /> ссылки на шаблон создаются путём передачи строкового значения как свойства в узле Virtual DOM:
...
В версиях до 3.5, где функция `useTemplateRef()` ещё не была введена, ссылки на шаблон создаются путём передачи самого `ref()` как свойства в узле Virtual DOM:
...
С помощью Options API ссылки на элементы шаблона создаются путём передачи имени ссылки в виде строки в узле Virtual DOM:
...
Функциональные компоненты — это альтернативная форма компонентов, которые не имеют собственного состояния. Они действуют как чистые функции: параметры — внутрь, узлы Virtual DOM — наружу. Они отображаются без создания экземпляра компонента (т. е. нет `this`), и без обычных хуков жизненного цикла компонентов.
