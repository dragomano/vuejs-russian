...
API `ref()`, `computed()` и `watchEffect()` являются частью API Composition. Если вы до сих пор использовали только Options API с Vue, вы заметите, что Composition API ближе к тому, как работает система реактивности Vue под капотом. Фактически, во Vue 3 Options API реализован поверх API Composition. Все обращения к свойствам экземпляра компонента (`this`) вызывают геттеры/сеттеры для отслеживания реактивности, а такие свойства, как `watch` и `computed`, вызывают свои эквиваленты Composition API внутри компонента.

</div>

## Время выполнения и время компиляции {#runtime-vs-compile-time-reactivity}

Система реактивности Vue основана на времени выполнения: отслеживание и срабатывание происходит во время выполнения кода непосредственно в браузере. Плюсы реактивности во время выполнения в том, что она может работать без этапа сборки, и в том, что в ней меньше крайних случаев. С другой стороны, это накладывает на него ограничения, связанные с синтаксисом JavaScript, что приводит к необходимости использования контейнеров значений, таких как рефы Vue.
...
Параметры отладки вычисляемых свойств доступны только через функцию `computed()` из Composition API.
...
Наблюдатели, объявленные с использованием объектного синтаксиса, также поддерживают параметры `onTrack` и `onTrigger`:
