...
:::tip Примечание
Мы пометили `$props` и `$emit` как устаревшие, чтобы при получении `ref` на пользовательский элемент у нас не было соблазна использовать эти свойства, поскольку они предназначены только для проверки типов в случае с пользовательскими элементами. Эти свойства фактически не существуют в экземплярах пользовательских элементов.
:::
