...
:::tip Совет
Этот раздел предполагает базовые знания об API Composition. Если вы изучали Vue только с Options API, вы можете переключить стиль API на Composition API (с помощью переключателя в верхней части левой боковой панели) и перечитать главы [Основы реактивности](/guide/essentials/reactivity-fundamentals) и [Хуки жизненного цикла](/guide/essentials/lifecycle).
:::
...
Эти ограничения важны, поскольку именно в этих контекстах Vue может определить текущий активный экземпляр компонента. Доступ к активному экземпляру компонента необходим для:

1. Регистрации в нем хуков жизненного цикла.

2. Привязки к нему наблюдателей, чтобы они автоматически уничтожались при размонтировании экземпляра и предотвращали утечки памяти.
