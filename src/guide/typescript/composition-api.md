...
Это также работает, если `Props` импортируется из другого файла — будь то относительный импорт, путь через алиас (например, `@/types`) или внешняя зависимость (например, из `node_modules`). Данная возможность требует, чтобы TypeScript был установлен как **peer-зависимость** Vue.
...
Чтобы получить подсказки типов и проверку типов для глобальных пользовательских директив, объявленных с помощью `app.directive()`, вы можете расширить `GlobalDirectives`:
