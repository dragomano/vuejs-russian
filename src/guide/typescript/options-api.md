...
:::tip Совет
Хотя Vue поддерживает использование TypeScript с помощью API Options, рекомендуется использовать Vue с TypeScript через API Composition, так как он предлагает более простой, эффективный и надежный вывод типов.
:::

## Типизация входных параметров компонента {#typing-component-props}

Вывод типа для параметров в Options API требует обёртывания компонента с помощью `defineComponent()`. С его помощью Vue может определять типы параметров на основе свойства `props`, принимая во внимание дополнительные параметры, такие как `require: true` и `default`:
...
Для аннотирования сложных типов параметров мы можем использовать служебный тип `PropType`:
...
- [Модульные тесты TypeScript для расширений типов компонентов](https://github.com/vuejs/core/blob/main/packages/dts-test/componentTypeExtensions.test-d.tsx)
