...
- Чтобы получить пошаговое руководство по рекомендуемой настройке, посмотрите этот интерактивный [урок на Scrimba](https://scrimba.com/links/vue-quickstart), который покажет вам, как запустить, отредактировать и развернуть ваше первое приложение на Vue.
...
- Установка [Node.js](https://nodejs.org/) версии `^22.18.0 || >=24.12.0`
  :::

В этом разделе мы расскажем, как создать [одностраничное приложение](/guide/extras/ways-of-using-vue#single-page-application-spa) Vue на вашей локальной машине. Созданный проект будет использовать настройку сборки на основе [Vite](https://vite-docs.ru/) и позволит нам использовать [однофайловые компоненты](/guide/scaling-up/sfc) (SFC).
...
- Чтобы узнать больше о базовом инструменте сборки Vite, ознакомьтесь с [Документацией Vite](https://vite-docs.ru/).
...
В остальной части документации мы будем использовать преимущественно синтаксис [ES-модулей](https://developer.mozilla.org/ru/docs/Web/JavaScript/Guide/Modules). Большинство современных браузеров теперь поддерживают ES-модули по умолчанию, поэтому мы можем использовать Vue из CDN через нативные ES-модули, как здесь:
...
Существуют фреймворки Vue, которые из коробки поддерживают [SSR](/guide/scaling-up/ssr) и другие возможности:
- [Nuxt](https://nuxt.com/)
- [Vike](https://vike.dev/)
...
:::tip Совет
Общая рекомендация — использовать фреймворк только в том случае, если вам нужен SSR.

Если SSR вам не нужен, можно просто использовать [Vite](https://vite-docs.ru/) (именно это настраивается в разделе [Создание приложения](#creating-a-vue-application)).
:::

:::info Информация
Фреймворки Vue обычно используют Vite под капотом, поэтому прямое использование Vite вместо фреймворка Vue является более простым вариантом, если вам не нужен SSR. При этом фреймворки также предоставляют дополнительные возможности, например UI-темы, что тоже может быть причиной выбрать фреймворк Vue вместо одного лишь Vite.
:::
