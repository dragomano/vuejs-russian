...
В Vue Router встроена поддержка [ленивой загрузки компонентов](https://vue-router-ru.netlify.app/guide/advanced/lazy-loading.html) с помощью динамического импорта. Они отличаются от асинхронных компонентов и в настоящее время не вызывают `<Suspense>`. Однако они всё ещё могут иметь асинхронные компоненты в качестве потомков, и те могут вызывать `<Suspense>` обычным способом.
