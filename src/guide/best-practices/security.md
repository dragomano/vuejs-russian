...
Мы настоятельно рекомендуем никогда не рендерить элемент `<script>` во Vue, поскольку шаблоны и рендер-функции никогда не должны иметь побочных эффектов. Однако это не единственный способ включить строки, которые во время выполнения будут анализироваться как JavaScript.
