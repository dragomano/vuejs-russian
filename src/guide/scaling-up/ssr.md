...
Приложение Vue.js с серверным рендерингом также можно считать «изоморфным» или «универсальным», в том смысле, что большая часть кода вашего приложения выполняется на сервере **и** клиенте.
...
```js [app.js]
// (совместно используемый сервером и клиентом)
import { createSSRApp } from 'vue'
...
Полноценная реализация была бы довольно сложной и зависит от выбранного вами инструментария сборки. Поэтому мы настоятельно рекомендуем использовать [фреймворки Vue](/guide/quick-start#frameworks), если вам нужен SSR, поскольку они зачастую имеют встроенную поддержку SSR.
