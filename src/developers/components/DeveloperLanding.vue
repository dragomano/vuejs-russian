...
const hireUsLink = computed(() =>
  generateUTMUrl(partnerConfig.hireUsButtonUrl, route.path)
)
...
onMounted(() => {
  spotlightedProfile.value = allDevelopers.value.length
    ? allDevelopers.value[
        Math.floor(Math.random() * allDevelopers.value.length)
      ]
    : null
})
</script>
...
<template>
  <PageShowcaseListLayout
    spotlightTitle="Обзор"
    featuredTitle="Сертифицированные разработчики Vue.js"
  >
    <template #hero>
      <DeveloperHero />
    </template>
    ...
    <template #spotlight>
      <DeveloperCard
        v-if="spotlightedProfile"
        hero
        :data="spotlightedProfile"
      />
    </template>
    ...
    <template #actions>
      <a
        ...
        class="accent-button"
      >
        Связаться с {{ partnerConfig.partnerName }} для индивидуального
        подбора
      </a>
    </template>
