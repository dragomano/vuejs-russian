...
- Поскольку эти компоненты используются так часто, вы можете просто сделать их глобальными, а не импортировать их повсюду. Префикс делает это возможным с помощью Vite:
...
К сожалению, в HTML нельзя использовать самозакрывающуюся запись для пользовательских компонентов (например, `<MyComponent />`). Такая запись разрешена только для ограниченного набора встроенных элементов, которые называются [«пустыми» (void elements)](https://html.spec.whatwg.org/multipage/syntax.html#void-elements) — например, `<img>` или `<br>`. Именно поэтому описанная стратегия работает только тогда, когда компилятор шаблонов Vue может обработать шаблон **до того, как этот шаблон попадёт в DOM**. В этом случае Vue успевает сгенерировать корректный HTML, который полностью соответствует спецификации DOM.
